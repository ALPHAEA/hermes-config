...
- Obsidian 笔记规范（NOTE 目录）：
  - 分类维度：按技术/主题分目录（Java/, Network/, SmartHome/, IDE/ 等），可自动新增或拆分二级目录
  - 文件名：【source】title.md，从 frontmatter 取值。title 中 /→-，:→：，?→？，*→·，"→'，<→《，>→》 ，|→-
  - frontmatter 模板：title / source / source_url / author / created / tags / assets
...
- 系统: Linux 容器环境
- Skills: 96 个 SKILL.md 文件，覆盖 27 个分类
- Config: 393 行，_config_version: 21
- Cron: 3 个定时任务（上海松江天气、每日AI科技早报、daily-hermes-config-sync）
- GitHub 仓库: https://github.com/ALPHAEA/hermes-config.git (每日同步)
- 平台接入: 飞书 (Feishu)
- 当前个性: kawaii
