...
```bash
# 直接使用（本包自带 src/ 引擎）
node src/bin/verify.js          # 验证安装（若含 bin）
# 或作为库：
const hf = require('./src/index.js');   # 46 维判别函数
const gate = require('./src/gate.js');  # 完整门禁管线
```
...
gate.checkInput(text)   // 判别用户输入（scope/premise/46维）
gate.checkDraft(text)   // 判别 AI 草稿
gate.checkOutput(text)  // 判别 AI 输出（发送前）
gate.runPipeline({ input, mode, anchor })  // 完整管线
```
...
hf.checkHateSpeech(text)          // 仇恨言论
hf.checkEmotionalManipulation(t)  // 情绪操控
hf.checkDoubleBind(text)          // 双重束缚
hf.checkContradiction(text)       // 自相矛盾
hf.discriminate(text)             // 46 维全量判别 → {gate.action}
```
